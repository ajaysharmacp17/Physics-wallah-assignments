...
CREATE TABLE Student_Performance ( student_id INT PRIMARY KEY, name VARCHAR(50), course VARCHAR(30),score INT,
...
Row_number() over (order by score desc) As highest_score
...
select *,
lag (score) over( order by score desc) Previous_student_score
from student_performance;
...
select *,
ntile(4) over( order by score desc) as Top_Performer
from student_performance;
...
select *,
row_number() over( partition by course order by attendance desc) As highest_attendance_course_wise
from student_performance;
